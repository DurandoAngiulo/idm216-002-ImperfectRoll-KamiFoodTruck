<div class=" mobile-navbar d-flex w-100 justify-content-between px-5 pt-1">
    <a class="py-1 d-inline-block" href="<?php echo site_url(); ?>/pages/home.php">
        <div class="d-flex flex-wrap justify-content-center"> 
            <img class="mobile-nav-image" src="<?php echo site_url(); ?>/dist/images/icons/Home.png">
        </div>
        <p class="pt-1 text-center">Home</p>
    </a>
    <a class="pt-1 d-inline-block" href="<?php echo site_url(); ?>/pages/order-confirmation.php">
        <div class="d-flex justify-content-center"> 
            <img class="mobile-nav-image" src="<?php echo site_url(); ?>/dist/images/icons/Orders.png">
        </div>
        <p class="pt-1 text-center">Orders</p>
    </a>
    <a class="pt-1 d-inline-block" href="<?php echo site_url(); ?>/pages/cart.php">
        <div class="d-flex flex-wrap justify-content-center"> 
            <img class="mobile-nav-image" src="<?php echo site_url(); ?>/dist/images/icons/Cart.png">
        </div>
        <p class="pt-1 text-center">Cart</p>
    </a>
</div>
